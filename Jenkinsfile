        ...
        stage('Build') {
            steps {
                script {
                    ...
                    sh 'docker images'
                    // Tag the image
                    sh 'docker tag reactjs-demo:latest sravanaboyanagayathri/prodrepo:latest'
                    ...
                    withCredentials([
                        ...
                        sh 'docker push sravanaboyanagayathri/prodrepo:latest'
                    }

                    // Remove local images
                    sh 'docker rmi -f reactjs-demo:latest'
                    sh 'docker rmi -f sravanaboyanagayathri/prodrepo:latest'
                }
            }
        }
        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    // Pull the image
                    sh 'docker pull sravanaboyanagayathri/prodrepo:latest'

                    // Run the container
                    sh 'docker run -d -p 80:80 --name reactjscontainer sravanaboyana/prodrepo:latest'
                }
            }
        }
    }
